"""
ShadowKey Phase 3 - Data Storage Module
Handles SQLite database operations for multi-user sessions, keystrokes, features, and ML predictions.
Includes authentication and cloud sync support.
"""
...
class DataStorage:
    ...
    def initialize_database(self) -> None:
        ...
        # Users table - Phase 3: Added password_hash, last_login, cloud_user_id
        cursor.execute("""
        ...
        # Sessions table - Phase 3: Added sync_status
        cursor.execute("""
    ...
    def create_user(self, username: str, password: str = None) -> int:
        """
    ...
    def get_pending_sync_sessions(self, user_id: int) -> List[Dict[str, Any]]:
        """
        Get all sessions that need cloud sync.
        
        Args:
            user_id: ID of user
            
        Returns:
            List of session dictionaries with sync_status='pending'
        """
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT * FROM sessions
            WHERE user_id = ? AND sync_status = 'pending' AND end_time IS NOT NULL
            ORDER BY start_time DESC
        """, (user_id,))
    ...
    def mark_session_synced(self, session_id: int) -> None:
        """
        Mark a session as synced to cloud.
        
        Args:
            session_id: ID of session to mark as synced
        """
        cursor = self.conn.cursor()
        cursor.execute(
    ...
    def mark_session_sync_failed(self, session_id: int) -> None:
        """
        Mark a session sync as failed.
        
        Args:
            session_id: ID of session to mark as failed
        """
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE sessions SET sync_status = 'failed' WHERE session_id = ?",
            (session_id,)
        )
        self.conn.commit()
