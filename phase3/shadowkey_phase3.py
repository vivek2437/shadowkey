"""
ShadowKey Phase 3 - Main Application
Multi-user keystroke authentication with cloud sync and advanced ML features.
"""
...
import ctypes
try:
    # Fix 1: Disable High-DPI scaling to prevent invisible windows
...
class ShadowKeyApp:
    """Main application class for ShadowKey Phase 3 with multi-user support."""
    
    def __init__(self, root: tk.Tk, user_id: int, username: str, config_path: str = "config_phase3.json"):
        """
        ...
        """
        self.root = root
        self.root.title(f"ShadowKey Phase 3 - {username}")
        ...
        self.cloud_manager = CloudSyncManager(self.config['cloud']['storage_dir'])
    ...
    def _create_widgets(self) -> None:
        ...
        self.sync_btn = tk.Button(
            control_frame, text="☁️ Sync Cloud", command=self._sync_to_cloud,
        ...
        self.user_label = tk.Label(
            stats_frame, text=f"Logged in as: {self.current_username}", bg='#ecf0f1',
            font=('Arial', 10, 'bold'), anchor='w', fg='#27ae60'
        )
...
Session saved to database (ID: {self.current_session_id})
    ...
    def _show_about(self) -> None:
        """Show about dialog."""
        messagebox.showinfo(
            "About ShadowKey Phase 3",
            "ShadowKey Phase 3\n\n"
            "Multi-user keystroke authentication with cloud sync\n"
            "and advanced ML-based behavioral biometrics.\n\n"
            "Features:\n"
            "• Multi-user login/registration\n"
            "• Per-user ML model isolation\n"
    ...
    def _sync_to_cloud(self) -> None:
        ...
        try:
            ...
            if not pending:
                messagebox.showinfo("Cloud Sync", "No sessions to sync.")
                return
            ...
            for session in pending:
                ...
                session_data = {
                    ...
                    'keystrokes': self.db.get_session_keystrokes(session['session_id']),
                    'features': self.db.get_session_features(session['session_id'])
                }
                
                # Sync to cloud
                if self.cloud_manager.sync_up(self.current_user_id, session_data):
                    self.db.mark_session_synced(session['session_id'])
                    synced_count += 1
            
            messagebox.showinfo(
                "Cloud Sync Complete",
                f"Successfully synced {synced_count} session(s) to cloud.\n\n"
                f"Cloud storage: {self.config['cloud']['storage_dir']}/user_{self.current_user_id}/"
            )
    ...
    def _logout(self) -> None:
        ...
        if self.is_session_active:
            if messagebox.askyesno("Session Active", "A session is active. Stop and logout?"):
                self._stop_session()
            else:
                return
...
def show_login_and_start(root):
    """Show login dialog and start app if successful."""
    root.withdraw()  # Hide root window initially
    
    db = DataStorage()
    login = LoginDialog(root, db, min_password_length=4)
